...
{
  print(paste("percent notes's note type id in fields's ids:",
            round(sum(note$ntid %in% fields$ntid)/length(note$ntid),3)))
  fields <- fields %>%
    group_by(ntid) %>%
    summarise(ord.max=max(ord))
  note<-merge(note,fields,all.x = T,all.y=F)
...
cards<-merge(cards,note[,c("nid","card.txt","ord.max","text.cue.cleaned")],all.x = T,all.y=F)
...
if(any(crd$did<10000)){
  ...
  crd<-crd[crd$did>10000,]
    ...
    medek <- median(sort(as.numeric(crd$did))) - 300000000
...
if(any(crd$cid<10000)){
  ...
  crd<-crd[crd$cid>10000,]
    ...
    medek <- median(sort(as.numeric(crd$cid)))
