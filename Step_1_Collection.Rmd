...
This step is mostly debugging and will not be interesting to anyone who is not analyzing their own data. Please notice the amount of code needed to remove all the bugs in Anki's database and look at the pretty latter plots. If you are running this on your own data you should check the output of each and every code section carefully. Look for label CHANGEME, which for the following 3 notebooks are mostly errors.
...
require(DBI)
#CHANGEME
COLLECTION_PATH <- "collection8_10_23.anki2"
...
st <- function(nic) {
  tbp <- sort(table(nic),decreasing=T)
...
Columns probably for syncing. Removing most. Plot of when cards modified.
...
Filtered deck ids replaced with original deck ids. As main meaning of 'deck' is similar subject not schedule this is obviously right. Not sure about due though.
...
Cleaning text function.   Clean deck names as well as card text. CHANGEME if bug.
```{r clean deck names}
...
Clean and join several tables into templates.
```{r merge many tables into templates}
{
...
stopifnot(all(notetypes$id %in% templates$ntid),
...
typ<-c()
for(i in fields$config) try({typ<-c(typ,(rawToChar(unlist(unlist(i)))))})
fields$config <- typ
#fields$config <- NULL
names(fields)[names(fields)=="config"] <- "config_notes"
...
stopifnot(all(templates$ntid %in% fields$ntid),
...
Merge all useful info into a single object (sheet). Bug checks too. really two; crd and rev. 
...
```
Anki's own statistics say cards unseen are .244 percent or (56832 - 42990) 13842 / 56832 . It counts notes not cards. See Section TIMESTAMP.
...
Duplicate decks! if greater than 90% duplicated, recommend deleting one of them anyway. On the other hand making duplicate decks is an quick way to manipulate the Subject detection algorithm.
```{r Duplicat decks}
 crd %>%
  group_by(card.txt.old) %>%
...
When were cards added that have ever been reviewed? Good and bad decisions? INTERESTING
...
SELF REVIEW  In first 2 months additions were well considered enough but then got obsessed with additions. Somewhere around 2021-10 this stopped and additions became better planned again.
