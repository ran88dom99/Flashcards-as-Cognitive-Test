...
Later notebooks will benefit from using the subject of a deck instead of the deck name or folder heirarchy. At least for big collections with mixed subject decks. Several things may indicate subject similarity; when the decks were added, when they were reviewed, where in the folder of deck heirarchy did user put each deck and what rare words are in the cards of the deck. The first two are not actualy very useful. Text mining is complicated and user should have an easy way to give input so user's deck hierarchy takes precedence over word coincidence. 
...
I imagine many people organize their decks by what they intend to do with the deck rather than by subject matter. Specifically they could have in their collection lowest level deck of 'trashed', 'highest importance', 'learning' and 'maintaining' and plenty of single subject folders. App splits all folders found to be too multisubject, then the decks these split folders held are added to low level single subject folders. LL SS folders with too much in common are combined with other LL SS folders into Subjects. During process original folder hierarchy is unaltered and kept separately. 

The output of this notebook can vary depending on small changes to U's collection. Effect is most drastic when the final anchor decks merge. Running subject detection algorithm on own data will still help User understand their collection and habits. Users applying app to their own data should start checking that the output is reasonable at '## Decks rearranged for Subject' .
...
Approximate subject by assuming lowest level deck is the subject then putting any set less than 100 cards into 'other'. Will be superseded by 'Subject'. Number of cards per deck lowest deck. 
warning to self is this is broken then heirarchy not in crd yet
...
# look at the most frequent bi-grams, and least frequent just for double check
tf_bigrams <- tf_mat[ stringr::str_detect(tf_mat$term, "_") , ]
head(tf_bigrams[ order(tf_bigrams$term_freq, decreasing = TRUE) , ], 20)
head(tf_bigrams[ order(tf_bigrams$term_freq, decreasing = FALSE) , ], 6)
...
Haberman formula takes in two dichotomous variables, calculates their coincidence, and outputs a similarity distance (or how unlikely that there is no relation between them) in standard deviations. 
Haberman formula is applied to word incidence to detect the similarity in subject matter between two decks. If bug CHANGEME.
```{r haberman}
{
require(netCoin)
...
crd$subject.folder.split.level <- 1
...
crd$subject.folder.split.level[change.crd] <- folder.lv+1
...
deks.reattach <- unique(crd$subject.category[crd$subject.folder.split.level > 1])
deks.main <- unique(crd$subject.category[crd$subject.folder.split.level == 1])
...
```{r ignore old method of dtm to subject and verification identical}
...
metest$subject.anchor.folder <- metest$subject.category
metest$subject.category <- NULL
#subject.anchor.folder is the anchor before merging anchors . Probably should be removed
crd <- metest
metest <- NULL
```
...
```{r check that every crd got a subject}
...
try({
  N <- merge(Ne,mor_join,all.x=T,by.y="dek.nam",by.x="name")
...
Quantile quantile plot of similarity  between decks of same subjects and similarity  between decks of different subjects.
Quantile-Quantile plot has nearly constant slope with internal (decks in same subject) coincidence reaching much higher than external. Internal coincidence is not significantly negative either. So I guess this is decent convergent vs divergent validation of the new clusters?
...
Single subject per plot. Histogram of reviews colored by new or already seen cards. Three curves made of points of different colors describing cumulative sum of cards, added, added and later will be seen, and currently seen.
