...
Haberman formula is applied to word incidence to detect the similarity in subject matter between two decks.
```{r haberman}
{
require(netCoin)
...
Find clusters of decks that have very very similar subjects, or at least vocabularies. Could be important interesting for understanding collection detecting decks that may as well be duplicates.
...
Treemaps by subject. Area is quantity.
...
ggplot(aes(x=milli_to_date(did),fill=subject))+
...
ggplot(rev)+
  geom_histogram(aes(x=milli_to_date(id),fill=subject),bins = 45)+
  xlab("Date") +
...
saveRDS(options(),"options_from_two.RDS")
gc()
